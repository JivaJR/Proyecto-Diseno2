...
export const ReactMapConsulta = ({lat,long,polyline}) => {
    ...
    const handleMapClick = (e) => {
        ...
        const newPolip = isInCircle(datesToCircle,clickedCoordinates)
        if(newPolip){
                    ...
                    ?<>
                        {
                            multiPoly.map((poly)=>{
                                ...
                                {
                                    poly.map((dato)=>(
                                        <Marker position={[dato[0].toString(),dato[1].toString()]} icon={CircleIcon}>

                                        </Marker>
                                    ))
                                }
                                <Marker position={[poly[0][0].toString(),poly[0][1].toString()]} icon={StartIcon} >
                                    <Popup><pre>inicio</pre></Popup>
                                </Marker>
                                <Marker position={[poly[poly.length-1][0].toString(),poly[poly.length-1][1].toString()]} icon={CircleIcon} >
                                    <Popup><pre>Fin</pre></Popup>
                                </Marker>
                                {/* <FinalMarker pos={[poly[poly.length-1][0].toString(),poly[poly.length-1][1].toString()]}/> */}
                                </>
                                :null
                                }
